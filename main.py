...
# Config 
class Config(object):
    BOT_TOKEN = os.environ.get("BOT_TOKEN", "7982445331:AAHRPBFbpCpWcSV4Yl8svwhuwKGuHySTb90")
    API_ID = int(os.environ.get("API_ID",  "23671274"))
    API_HASH = os.environ.get("API_HASH", "09b9c07a023f7c13c2164f2b22bd937e")
    DOWNLOAD_LOCATION = "./DOWNLOADS"
    SESSIONS = "./SESSIONS"

    AUTH_USERS = os.environ.get('AUTH_USERS', '903077627').split(',')
    for i in range(len(AUTH_USERS)):
        AUTH_USERS[i] = int(AUTH_USERS[i])

    GROUPS = os.environ.get('GROUPS', '-1002484508792').split(',')
    for i in range(len(GROUPS)):
        GROUPS[i] = int(GROUPS[i])

    LOG_CH = os.environ.get("LOG_CH", "-1002484508792")
